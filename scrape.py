...
for stock in stocks_list:
    stock_dict[stock] = pd.DataFrame(columns = ['Date', 'Time', 'Price', '50-Day MA', '200-Day MA', 'Market Open', 'Prev Close', 'Trading Volume'])
...
def scrape(stocks = stocks_list, dictionary = stock_dict):
    for ticker in stocks_list:
        ...
        # m_open_close = m_open + '/' + m_close

        # volume
        volume = float( soup.find_all("td", {"class":"Ta(end) Fw(600) Lh(14px)"})[6].find('span').text.replace(',', '') )

        fifty_day_SMA, t_hundred_day_SMA = 200, 200

        # append info
        entry = [date, time, price, fifty_day_SMA, t_hundred_day_SMA, m_open, p_close, volume]
        stock_dict[ticker] = stock_dict[ticker].append(pd.Series(entry, index=['Date', 'Time', 'Price', '50-Day MA', '200-Day MA', 'Market Open', \
                                                                               'Prev Close', 'Trading Volume']), ignore_index=True)
